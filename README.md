...
- `/resume`：用交互卡片按更新时间分页列出未归档的 Codex 历史会话（包括 App Server、CLI 和 IDE 会话），首屏只读取最近 5 个；恢复列表优先显示桥接当前运行态和 App Server 当前状态，历史轮次与 Goal 状态在选择会话时精确读取，因此不会把桥接正在执行的会话误显示为“已中断”；
- 恢复卡片按需显示“上一页”和“下一页”，每页 5 个会话按钮；页面通过 App Server 游标按需读取并在当前聊天内缓存，也可使用 `/resume prev|next`；
- `/resume 编号|标题`：继续当前列表中的历史会话，将当前飞书聊天绑定到该 thread，并切换到其工作目录；标题必须唯一匹配；已完成轮次会回放真正的最近一轮用户输入和最终答复，不重发历史附件；运行中的普通会话或 Goal 会话会显示已接入提示，并回放最近一条可读过程；随后实时转发后续过程消息，不显示空的历史输入或最终答复占位；
...
飞书发消息后，桥接会把任务注入共享 App Server，桌面端能看到该会话的实时消息流；反过来，桌面端（或其他非桥接客户端）在已绑定飞书的线程上发消息时，桥接也会把内容同步回飞书：

- 桥接启动时自动恢复对 `sessions.json` 中全部已绑定线程的订阅，因此无需先通过飞书发任务即可接收桌面消息。
- 桌面用户消息以 `[桌面]` 前缀的机器人新消息出现在对应飞书聊天；Codex 的中途 commentary（仅含中文的进度）和最终答复也会按顺序转发，桌面消息与 Codex 回复均不回复旧消息，而是发送新消息。
- 图片、文件等非文本输入在飞书侧显示 `[图片]`/`[文件]` 占位；本地 Markdown 图片链接沿用现有渲染链路处理。
- 桥接自己发起的任务不会进入镜像转发，审批和用户输入卡片仍由桌面端自行处理，不会重复或循环发送。
...
- Codex 的 `commentary` 进度（仅转发含中文或本地图片/链接的过程，避免英文过程刷屏）；
- 最终回答。

部分模型或新内核会把中文过程消息标成无 `phase` 的 `agentMessage`，桥接会把这类消息先缓存一条，再根据下一条事件判定：后续仍有工具、推理或新消息时按过程转发，紧接轮次结束时按最终答复发送，避免过程积压到轮次结束时一次性集中出现。多线程各自独立缓存，并行会话互不影响。

`reasoning` 项不转发，隐藏链式推理不会出现在飞书。

终端、文件修改、MCP、网页搜索等工具调用事件不会转发到飞书，避免过程消息过多。

所有已识别的斜杠命令和控制卡片都通过聊天级短路由队列处理，不等待正在运行的 Codex turn。普通任务在选路时固定 thread、目录、模型、思考强度和审批模式，再进入 thread 队列；同一 thread 严格串行，不同 thread 可并行。`/new` 或 `/resume` 切换选择后，旧 thread 的任务继续后台执行并保留历史，但不再向该飞书聊天推送进度、审批提示、最终答复、错误或附件；恢复一个仍在运行的 thread 后，桥接会重新加入该 thread 并接收此后的过程消息和最终答复。
...
计划模式产生最终计划后，桥接会为每个计划创建一份 `codex` 文件夹中的 Markdown 云文档，并在确认卡片中展示约 1200 字预览、完整计划链接、本地图片和操作按钮。“否决，继续修改”会保持计划模式并等待修改意见；“接受并执行”会切换到默认执行模式，并自动发送“执行刚刚确认的计划”。每个计划项只能处理一次，处理状态会持久化，因此重复点击、旧卡片、跨会话点击或服务重启都不会重复执行。计划正文中的 Markdown 图片语法会在卡片中替换为替代文本，避免飞书卡片因缺少 `image_key` 渲染失败；云文档目录或单次创建失败时保留原有卡片/长文本降级，卡片发送仍失败时会回退发送计划正文，并要求直接回复同意或修改意见。
...
- `modelSettings`：聊天到后续轮次的模型和思考强度策略（由 `/model`、`/think`、`/work` 更新）；
